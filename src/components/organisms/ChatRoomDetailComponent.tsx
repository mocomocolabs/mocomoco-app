...
import { ChatMessage } from '../atoms/ChatMessageComponent'
import { ProfileImage } from '../atoms/ProfileImageComponent'
import { TextXs } from '../atoms/TextXsComponent'
import { ChatImageGrid } from '../molecules/ChatImageGridComponent'
    ...
    >
      <Observer>
        {() => {
          ...
          // 2) 동시에, chats에 insert해야 함? 그 찰나에 length가 변경되는 걸 막을 수 있을까? block 시킬 수 있나?
                    ...
                    <div className='flex-col ml-2'>
                      ...
                      <div className={`${isMe ? 'flex-row-reverse' : 'flex'} items-end mt-1 gap-1`}>
                        {v.imageUrls?.length > 0 ? (
                          <ChatImageGrid imageUrls={v.imageUrls} uploading={v.type === ChatType.UPLOADING} />
                        ) : (
                          <ChatMessage message={v.message} isMe={isMe} />
                        )}
